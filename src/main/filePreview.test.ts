...
describe("repository preview assets", () => {
  it("reads working, staged, and committed SVG bytes while preserving text diffs", async () => {
    ...
    await fs.writeFile(path.join(dir, "mark.svg"), svg("committed"));
    await git("add", "mark.svg");
    await git("-c", "user.name=Preview Test", "-c", "user.email=preview@example.test", "commit", "-m", "fixture");
    const hash = await git("rev-parse", "HEAD");
    await fs.writeFile(path.join(dir, "mark.svg"), svg("staged"));
    await git("add", "mark.svg");
    ...
    for (const [source, expected] of [
      ...
      const image = await service.getFilePreviewImage({ repoPath: dir, path: "mark.svg", source });
      expect(image.mimeType).toBe("image/svg+xml");
      expect(Buffer.from(image.data).toString()).toBe(svg(expected));
    }
