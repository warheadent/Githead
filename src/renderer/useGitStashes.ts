...
export function useGitStashes(repoPath: string, enabled: boolean, active: boolean) {
  ...
  useEffect(() => {
    if (!active || state.loadedRepoPath !== repoPath || state.loading || state.entries.length === 0) return;
  ...
  }, [active, repoPath, select, state.details?.stash.ref, state.detailsLoading, state.entries, state.loadedRepoPath, state.loading, state.selectedRef]);
