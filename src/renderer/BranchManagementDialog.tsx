  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-h-[85vh] overflow-hidden sm:max-w-3xl" aria-busy={busy}>
      ...
      <MotionSwap item={{ key: mode.kind, content: mode.kind === "list" ? <div className="grid min-h-0 gap-4 overflow-hidden">
        ...
        <div className="grid min-h-0 grid-cols-[minmax(0,1fr)] gap-2 overflow-auto" role="list" aria-label="Local branches">
          {!visibleBranches.length ? <p className="py-8 text-center text-sm text-muted-foreground">{branches.length ? "No branches match your search." : "No local branches found."}</p> : visibleBranches.map((branch) => <div key={branch.name} role="listitem" className="flex items-center gap-3 rounded-md border p-3">
            ...
            <div className="min-w-0 flex-1"><div className="flex items-center gap-2"><TooltipTarget content={branch.name}><span className="truncate font-medium">{branch.name}</span></TooltipTarget>{branch.current ? <Badge>Current</Badge> : null}{branch.worktreePath && !branch.current ? <Badge variant="outline">Worktree</Badge> : null}</div>{branch.worktreePath && !branch.current ? <TooltipTarget content={branch.worktreePath}><p className="truncate text-xs text-muted-foreground">{branch.worktreePath}</p></TooltipTarget> : branch.upstream ? <TooltipTarget content={branch.upstream}><p className="truncate text-xs text-muted-foreground">{branch.upstream}</p></TooltipTarget> : null}</div>
            {capabilities.renameBranches ? <TooltipButton type="button" variant="ghost" size="icon" disabled={busy || Boolean(branch.worktreePath && !branch.current)} aria-label={`Rename ${branch.name}`} tooltip={`Rename ${branch.name}`} disabledTooltip={branch.worktreePath && !branch.current ? "Open this worktree before renaming its branch" : undefined} onClick={() => beginRename(branch)}><Pencil /></TooltipButton> : null}
            {capabilities.removeBranches ? <TooltipButton type="button" variant="ghost" size="icon" disabled={busy || branch.current || Boolean(branch.worktreePath)} aria-label={`${archive ? "Archive" : "Delete"} ${branch.name}`} tooltip={`${archive ? "Archive" : "Delete"} ${branch.name}`} disabledTooltip={branch.worktreePath ? `Remove or switch the worktree using this branch first` : branch.current ? `Switch to another branch before ${archive ? "archiving" : "deleting"} this branch` : undefined} onClick={() => { setError(""); setForceDelete(false); setMode({ kind: "remove", branch }); }}>{archive ? <Archive /> : <Trash2 />}</TooltipButton> : null}
          </div>)}
        </div>
